...
import { getToday, getClientIP, sanitizeData } from '../utils/util.js'
...
export const login = async (req, res, next) => {
    try {
        ...
        res.cookie('access_token', token, {
            ...
            .status(200)
            .json({ _id: user._id, employeeId: user.employeeId, name: user.name, email, isAdmin: user.isAdmin, department, rank, regular, beginDate, where, hash })
    } catch (err) {
        console.log(err)
        next(err)
    }
